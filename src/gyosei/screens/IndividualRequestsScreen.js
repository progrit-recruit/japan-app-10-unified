...
import { individualRequests } from '../data/mockData';

const FILTERS = ['all', 'new', 'in_progress', 'waiting_docs', 'completed'];
...
function RequestCard({ item, lang, onPress }) {
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <View style={styles.clientInfo}>
          ...
          <Text style={styles.nationality}>{item.nationality} · {item.visaType}</Text>
        </View>
        {isUrgent && (
...
export default function IndividualRequestsScreen({ lang, navigate }) {
  const [activeFilter, setActiveFilter] = useState('all');
  const [searchText, setSearchText] = useState('');

  const filtered = individualRequests.filter(r => {
    ...
    const matchSearch =
      ...
      r.nationality.includes(searchText);
  ...
  const filterCount = (f) => {
    if (f === 'all') return individualRequests.length;
    return individualRequests.filter(r => r.status === f).length;
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.headerTitle}>{t(lang, 'individual_requests')}</Text>
        </View>
        <View style={styles.totalBadge}>
          <Text style={styles.totalBadgeText}>{individualRequests.length}</Text>
        </View>
      </View>
